...
// Enhanced Main Slideshow functionality
let mainCurrentSlide = 1;
let mainTotalSlides = 8;
let mainIsPlaying = true;
...
function initMainSlideshow() {
    ...
    // Event listeners
    document.getElementById('mainPrevBtn').addEventListener('click', mainPreviousSlide);
    document.getElementById('mainNextBtn').addEventListener('click', mainNextSlide);
    document.getElementById('mainPlayPauseBtn').addEventListener('click', mainTogglePlayPause);
    
    // Indicator clicks
    document.querySelectorAll('.main-indicator').forEach((indicator, index) => {
        indicator.addEventListener('click', () => mainGoToSlide(index + 1));
    });
    
    // Keyboard controls
    document.addEventListener('keydown', (e) => {
        if (e.key === 'ArrowLeft') mainPreviousSlide();
        if (e.key === 'ArrowRight' || e.key === ' ') {
            e.preventDefault();
            mainNextSlide();
        }
        if (e.key === 'Escape') mainTogglePlayPause();
    });
}

function startMainSlideshow() {
    if (mainSlideInterval) clearInterval(mainSlideInterval);
    if (mainProgressInterval) clearInterval(mainProgressInterval);
    ...
    mainProgressInterval = setInterval(() => {
        ...
        document.getElementById('mainProgressBar').style.width = mainProgress + '%';
    }, 10);
}
...
function mainNextSlide() {
    mainCurrentSlide = mainCurrentSlide >= mainTotalSlides ? 1 : mainCurrentSlide + 1;
    showMainSlide(mainCurrentSlide);
    mainProgress = 0;
}

function mainPreviousSlide() {
    mainCurrentSlide = mainCurrentSlide <= 1 ? mainTotalSlides : mainCurrentSlide - 1;
    showMainSlide(mainCurrentSlide);
    mainProgress = 0;
}

function mainGoToSlide(slideNum) {
    mainCurrentSlide = slideNum;
    showMainSlide(mainCurrentSlide);
    mainProgress = 0;
}

function showMainSlide(slideNum) {
    // Hide all slides
    document.querySelectorAll('.main-slide').forEach(slide => {
        slide.classList.remove('active');
    });
    
    // Show current slide
    const targetSlide = document.querySelector(`.main-slide[data-slide="${slideNum}"]`);
    if (targetSlide) {
...
function updateMainSlideCounter() {
    document.getElementById('mainCurrentSlideNum').textContent = mainCurrentSlide;
}

// Initialize main slideshow when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
...
// Pause main slideshow when user scrolls away from section
let mainSlideshowSection = null;

window.addEventListener('scroll', () => {
    if (!mainSlideshowSection) {
        mainSlideshowSection = document.querySelector('.slideshow-section');
    }
    
    if (mainSlideshowSection) {
        const rect = mainSlideshowSection.getBoundingClientRect();
        const isVisible = rect.top < window.innerHeight && rect.bottom > 0;
        
        if (!isVisible && mainIsPlaying) {
            stopMainSlideshow();
        } else if (isVisible && mainIsPlaying && !mainSlideInterval) {
            startMainSlideshow();
        }
    }
});
